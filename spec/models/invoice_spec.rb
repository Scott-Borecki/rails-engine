...
RSpec.describe Invoice, type: :model do
  ...
  describe 'class methods' do
    let!(:invoice1) { create(:invoice, status: 'packaged') } # transaction: success
    let!(:invoice2) { create(:invoice, status: 'shipped') }  # transaction: success
    let!(:invoice3) { create(:invoice, status: 'shipped') }  # transaction: success
    let!(:invoice4) { create(:invoice, status: 'returned') } # transaction: refunded
    let!(:invoice5) { create(:invoice, status: 'shipped') }  # transaction: failed
    ...
    describe '.total_revenue_generated' do
      before do
        create(:invoice_item, invoice: invoice1)                                 # Revenue = 0
        ...
        create(:invoice_item, invoice: invoice3, quantity: 25, unit_price: 8.99) # Revenue = 224.75
        create(:invoice_item, invoice: invoice3, quantity: 25, unit_price: 8.99) # Revenue = 224.75
        create(:invoice_item, invoice: invoice4)                                 # Revenue = 0
        create(:invoice_item, invoice: invoice5)                                 # Revenue = 0
      end
