...
RSpec.describe Merchant, type: :model do
  describe '.top_by_revenue' do
    ...
    let!(:merchant1) { merchant_with_revenue(ii_count: 1) }
    let!(:merchant2) { merchant_with_revenue(ii_count: 6) }
    let!(:merchant3) { merchant_with_revenue(ii_count: 4) }
    let!(:merchant4) { merchant_with_revenue(ii_count: 5) }
    let!(:merchant5) { merchant_with_revenue(ii_count: 3) }
    let!(:merchant6) { merchant_with_revenue(ii_count: 2) }
    let!(:merchant7) { merchant_without_revenue(ii_count: 7) }
    let!(:merchant8) { merchant_without_revenue(ii_count: 8) }
