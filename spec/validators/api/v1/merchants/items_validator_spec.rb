...
describe Api::V1::Merchants::ItemsValidator, type: :validator do
  describe 'object creation' do
    ...
    describe 'invalid objects' do
      let(:missing_merchant_id_message) { { merchant_id: ["can't be blank", 'is not a number'] } }
      let(:not_a_number_message) { { merchant_id: ['is not a number'] } }
      ...
      context 'when I do not provide an merchant id' do
        it 'returns an invalid object' do
          ...
          expect(validator.errors.messages).to eq(missing_merchant_id_message)
        end
      end
